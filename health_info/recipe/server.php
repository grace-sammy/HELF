...
if (isset($_POST['action'])) {
    ...
    case 'dislike':
         ...
         break;
    case 'unlike':
          $sql="DELETE FROM rating_health_info WHERE user_id='$user_id' AND post_id=$post_id AND b_code='레시피'";
          break;
    case 'undislike':
            $sql="DELETE FROM rating_health_info WHERE user_id='$user_id' AND post_id=$post_id AND b_code='레시피'";
      break;
...
function getDislikes($id)
{
    global $conn;
    $sql = "SELECT COUNT(*) FROM rating_health_info
          WHERE b_code='레시피' and post_id = $id AND rating_action='dislike'";
    $rs = mysqli_query($conn, $sql);
    $result = mysqli_fetch_array($rs);
    return $result[0];
}
...
function getRating($id)
{
    global $conn;
    $rating = array();
    $likes_query = "SELECT COUNT(*) FROM rating_health_info WHERE b_code='레시피' and post_id = $id AND rating_action='like'";
...
function userLiked($post_id)
{
    global $conn;
    global $user_id;
    $sql = "SELECT * FROM rating_health_info WHERE b_code='레시피' and user_id='$user_id'
...
$sql = "SELECT * FROM health_info where b_code='레시피' and num=$num"; //게시판 번호
